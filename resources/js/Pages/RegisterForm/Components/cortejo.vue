<script setup>
    ...
    const props = defineProps(['checklist', 'mobile', 'active']);
    ...
    const filteredAnswers = (question) => {
        return computed(() => {
            // Filtre as respostas com base em btnSelected e question
            if (btnSelected.value === null) {
                return question.answers;
            }
            // Adicione a lógica de filtragem aqui com base em btnSelected
            // Exemplo:
            return question.answers.filter(answer => {
                if (btnSelected.value === 2 && answer.id == 12) {
                    if (form.value[question_quadro.value] == answer.id) {
                        form.value[question_quadro.value] = null;
                        form.value[question_trajes.value] = null;
                    }
                    return false;
                }
                if (btnSelected.value === 3 && (answer.id == 12 || answer.id == 13 || answer.id == 14)) {
                    return false;
                }
            // Filtre com base na condição de btnSelected
            // Exemplo: return answer.someProperty === btnSelected.value;
            return true; // Ajuste conforme sua lógica de filtragem
            });
        }).value;
    };
    ...
    const hideBtn = () => {
        ...
        if (btnSelected.value === 2) {
            if (!formGroup.value[btnSelected.value]) return false
            return formGroup.value[btnSelected.value].length > 0 ? true : false;
        }
        if (btnSelected.value === 3) {
            if (!formGroup.value[btnSelected.value]) return false
            return formGroup.value[btnSelected.value].length > 8 ? true : false;
    ...
    const disabledDate = (time) => {
        let start = new Date(1924, 6, 31).getTime(); // 30 de junho de 1924
        let end = new Date(2010, 11, 31).getTime(); // 31 de dezembro de 2010
    ...
    function getList() {
        ...
        formResource.getForm(id).then(data => {
            ...
            for (var j in questions.value) {
                ...
                if (questions.value[j].isSpecial) question_quadro.value = questions.value[j].id;
    ...
    const submit = async () => {
        ...
        if ((btnSelected.value == 2 && formGroup.value[btnSelected.value].length <= 0) || (btnSelected.value == 3 && formGroup.value[btnSelected.value].length < 2)) {
            ...
            displayMessage('Para esta inscrição deve conter no mínimo ' + part + ' participantes.', 'error');
            return;
        }
        loading.value = true;
        ...
        } else if(btnSelected.value == 1) {
            if (!formRef) {
                displayMessage('Não foi possível registar a sua inscrição. Tente novamente.', 'error');
                loading.value = false;
                return;
            }
    ...
    const formatStoreData = () => {
        // let formData = {};
        // formData['checklist'] = props.checklist;
        // formData['btn'] = btnSelected.value;
        // formData['answer'] = {};
        // formData['participants'] = {};
        let formData = new FormData();
        formData.append('checklist', props.checklist)
        formData.append('btn', btnSelected.value);
        ...
        for (let j in questions.value) {
            ...
            if (question.type_question === 'image') {
                ...
                if (selectedCountry.value[question.id]) {
                    let codeNumb = codes.value[selectedCountry.value[question.id]] ? codes
                        .value[selectedCountry.value[question.id]].code : '';
        ...
        for (var k in formGroup.value[btnSelected.value]) {
            ...
            for (var m in questionsForm.value) {
                ...
                if (countryPhone.value[k][questionsForm.value[m].id]) {
                    let codeNumb = codes.value[countryPhone.value[k][questionsForm.value[m].id]].code ? codes.value[countryPhone.value[k][questionsForm.value[m].id]].code : '';
                    value = '+' + codeNumb + ' ' + value;
                }
                // formData['participants'][k][questionsForm.value[m].id] = value;
                formData.append(`participants[${k}][${questionsForm.value[m].id}]`, value);
            }
        }

        answersResource.storeCortejoAnswers(formData).then(data => {
            if (data) {
                ElMessage({
    ...
    const checkCondition = () => {
        for (var i in questions.value) {
            ...
            if (question.isSpecialBoards && (form.value[question_quadro.value] == null || form.value[question_quadro.value] == '')){
                form.value[question.id] = null;
                final_value = false;
            }

            if (final_value == true && (question.id == 3 || question.id == 4 || question.id == 5 || question.id == 6) && (btnSelected.value == 3)) {
                final_value = false;
            }
    ...
    const generateRulesForm = () => {
            ...
            for (var j in qt) {
                if (qt[j].mandatory && conditionsForm.value[k + qt[j].id]) {
                    ...
                    if (qt[j].type_question == 'phone') {
                        ...
                        if (!countryPhone.value[idx]) {
                            countryPhone.value[idx] = {};
                        }
                        if (!countryPhone.value[idx][qt[j].id]) {
                            countryPhone.value[idx][qt[j].id] = null;
                        }
                    }
                    if (qt[j].type_question == 'email') {
                        values.push({
        ...
        rulesForm.value['conditionsForm'] = [{
            validator: (rule, value, callback) => {
                if (!value) {
                    return callback(new Error(`Deve ler e aceitar a política de privacidade`));
                } else {
            ...
            for (var i in questionsForm.value) {
                ...
                for (var j in question.conditions) {
                    if (question.conditions[j].rule == '==') {
                        ...
                        //     formGroupData.value[k + question.id] = null;
                        // }
                    }
                    if (question.conditions[j].rule == '!=') {
                        let value = formGroupData.value[k + question.conditions[j].question_id] != question.conditions[j].answer_id;
    ...
    const validateDateRange = (rule, value, callback) => {
        if (!value) {
            ...
            if ((btnSelected.value == 1 || btnSelected.value == 2) && (date < start || date > end)) {
    ...
    const changeStep = async() => {
        ...
        await formRef.value.validate((valid, fields) => {
            if (valid) {
                ...
                currentStep.value = 1;
                // TODO: Check if it's to add automatically more 2 particpants to give a total of 3 when it's grupo the btnselected!
            } else {
                displayMessage('Devem ser preenchidos os campos obrigatórios.', 'error');
                loading.value = false;
            }
        });
    }
    ...
    const answerDisabled = (answer, q_id) => {
        if (btnSelected.value == 1 && answer.description == "Traje de Noivo") {
            return true;
        }
        if (btnSelected.value == 2 && answer.description == "Traje de Noivo" && formGroupData && formGroupData.value['0' + q_id] == answer.id) return true;
        return false;
    }

    const answerDisabledParticipant = (answer, q_id) => {
        if (btnSelected.value == 2 && answer.description == "Traje de Noivo" && form.value[q_id] == answer.id) {
            return true;
        }
        return false;
    }
            ...
            <div v-else>
                ...
                <QRCodeComponent v-else :message="message"></QRCodeComponent> -->
                    <div v-if="!displayQRCode">
                        <div class="btnSelect">
                            <p>Qual a inscrição que pretende realizar?</p>
                            <el-button v-for="(bt, idx_bt) in btnToSelect" class="cortejoBtn sizeBtn" :key="idx_bt" type="primary" :plain="bt.active == 1 ? false : true" @click="changeBtInscricao(idx_bt)">{{ bt.description }}</el-button>
                        </div>
                        <el-form v-if="btnSelected !== null" :rules="rules" ref="formRef" class="formContainer" :model="form" label-width="auto" :label-position="labelPosition">
                                ...
                                <div v-for="(question, idx) in questions" :key="idx">
                                    <el-form-item v-if="conditions[question.id]" :label="question.description" :prop="question.id.toString()" class="opensansName">
                                        ...
                                        <div class="displayPhoneNumber"
                                            ...
                                            <el-select class="opensansName" v-model="selectedCountry[question.id]"
                                            ...
                                            <el-input class="opensansName" v-model="form[question.id]" />
                                        </div>
                                        <!-- Number -->
                                        <el-input class="opensansName" v-model="form[question.id]"
                                            v-if="question.type_question === 'number' && conditions[question.id]" />
                                        <!-- Date -->
                                        <el-date-picker v-if="question.type_question === 'date' && conditions[question.id] && (btnSelected == 1 || btnSelected == 2)"
                                            class="opensansName" v-model="form[question.id]" format="YYYY-MM-DD"
                                            value-format="YYYY-MM-DD" type="date" placeholder="Selecione um dia" 
                                            :disabled-date="disabledDate" />

                                        <el-date-picker v-if="question.type_question === 'date' && conditions[question.id] && (btnSelected == 3)"
                                            class="opensansName" v-model="form[question.id]" format="YYYY-MM-DD"
                                        ...
                                        <el-select style="width:75%" class="opensansName"
                                            ...
                                            <el-option v-for="answer in filteredAnswers(question)" class="opensansName"
                                                :key="answer.id" :label="answer.description"
                                                :value="answer.id"
                                        ...
                                        <el-select style="width:75%" class="opensansName"
                                            ...
                                            <el-option v-for="answer in question.answers[form[question_quadro]]" class="opensansName"
                                                :key="answer.id" :label="answer.description"
                                                :value="answer.id" :disabled="answerDisabled(answer, question.id)" />
                                        </el-select>
                                        <!-- Checkbox -->
                                        <el-checkbox class="opensansName"
                                            v-if="(question.type_question === 'checkbox')  && conditions[question.id] && question.multiple"
                                            v-model="form[question.id]" label="Option 1" />
                                        <el-radio-group class="opensansName"
                                            v-if="(question.type_question === 'checkbox')  && conditions[question.id] && !question.multiple"
                                        ...
                                        <el-autocomplete v-if="question.type_question === 'rancho'"
                                            v-model="form[question.id]"
                                        ...
                                        <!-- Image -->
                                        <el-upload v-if="question.type_question === 'image'"
                                            v-model:file-list="form[question.id]" :class="uploadClass" action=""
                                            :auto-upload="false" :on-preview="handlePreview" :on-remove="handleRemove"
                                            list-type="picture" :limit="3" accept="image/*" multiple
                                            @exceed="handleExceed">
                                            <el-button class="opensansName" :disabled="form[question.id].length >= 3"
                                                type="primary">Carregar Fotografia(s)</el-button>
                                        </el-upload>
                                        <div v-if="question.type_question === 'image'" style="width: 100%">
                                            <small v-if="btnSelected == 2">Nota: Devem ser carregadas as fotografias dos participantes da inscrição.</small>
                                            <small v-if="btnSelected == 3">Nota: Devem ser carregadas as fotografias de grupo com o máximo de elementos presentes.</small>
                                        </div>
                                        <!-- Textarea -->
                                            ...
                                            <el-form-item v-if="conditionsForm[idx_group.toString() + qt_form.id]" :label="qt_form.description" :prop="idx_group.toString() + qt_form.id.toString()" class="opensansName" >
                                                ...
                                                <el-input class="opensansName"
                                                    ...
                                                    <div class="displayPhoneNumber"
                                                        ...
                                                        <el-input class="opensansName" v-model="formGroupData[idx_group.toString() + qt_form.id.toString()]" />
                                                    </div>

                                                    <!-- Date -->
                                                    <el-date-picker v-if="qt_form.type_question === 'date' && conditionsForm[idx_group.toString() + qt_form.id.toString()] && btnSelected == 2"
                                                    ...
                                                    <el-date-picker v-if="qt_form.type_question === 'date' && conditionsForm[idx_group.toString() + qt_form.id.toString()] && btnSelected == 3"
                                                        class="opensansName" v-model="formGroupData[idx_group.toString() + qt_form.id.toString()]" format="YYYY-MM-DD"
                                                        value-format="YYYY-MM-DD" type="date" placeholder="Selecione um dia" @change="(val) => checkMaiorIdade(val, idx_group.toString() + qt_form.id.toString())" />
                                                        <!-- Country -->
                                                    <el-select style="width:75%" class="opensansName"
                                                    ...
                                                    <el-select style="width:75%" class="opensansName"
                                                        ...
                                                        <el-option v-for="answer_p in qt_form.answers[form[question_quadro]]" class="opensansName"
                                                            ...
                                                            :value="answer_p.id" :disabled="answerDisabledParticipant(answer_p, qt_form.id)" />
                                                    </el-select>
                                            </el-form-item>
                                        </div>
                                        <div v-if="btnSelected == 3" class="btnSelect">
                                            <el-button type="danger" @click="deleteData(idx_group)"><el-icon class="header-icon"><delete /></el-icon></el-button>
                                        </div>
                                    </el-card>
                                    <div v-if="!checkHidden && currentStep == 1 && !hideBtn()" class="btnSelect" style="margin-top: 10px !important">
                            ...
                            <div v-if="checkHidden && btnSelected === 1">
                                <el-form-item class="opensansName termsConditions" label="" prop="conditions">
