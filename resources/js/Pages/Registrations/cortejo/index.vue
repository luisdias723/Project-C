...
const getCounters = () => {
    regiResource.getCounter({form: 2}).then(response => {
        if (response) {
...
const goToDetails = (id) => {
    // router.push(`inscricoes/detalhes/${id}`);
    const route = router.resolve({ path: `cortejo/detalhes/${id}` });
...
const goToTrajes = () => {
    // router.push(`inscricoes/detalhes/${id}`);
    const route = router.resolve({ path: `inscricoes/trajes` });
    // Abra a URL em uma nova aba
    window.open(route.href, '_blank');
};
...
const handleRowClick = (row) => {
    const route = router.resolve({ path: `cortejo/detalhes/${row.id}` });
    // Abra a URL em uma nova aba
    window.open(route.href, '_blank');
}
        ...
        <el-row>
            ...
            <el-col :span="24" class="filterData">
                <!-- <el-button class="primaryRo addFilterMargin" v-loading="loading_opc" type="primary" @click="goToTrajes">
                    ...
                    Inscrições por Quadro
                </el-button> -->
                <el-button class="primaryRo addFilterMargin" v-loading="loading_opc" type="primary">
                    <el-icon class="el-icon--left"><View /></el-icon>
                    Inscrições por Quadro
                </el-button>
                <el-select
