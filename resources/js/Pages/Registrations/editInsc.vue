...
const updateData = () => {
  regResource.updateStatus(formData.value).then(response => {
    if (response) {
...
// const updateObs = () => {
//       regResource.saveObs(form.value).then(response => {
//         if (response) {
//           displayMessage('success', 'Observação adicionada com sucesso')
//           closeObsDialog()
//         } else {
//           displayMessage('error', 'Não foi possível adicionar a observação')
//         }
//       }).catch(error => {
//         console.log(error);
//         displayMessage('error','Não foi possível adicionar a observação')
...
const storeQR = (img) => {
  ...
  regResource.saveQRCode(dataToPass).then(response => {
...
const submit = () => {
  ...
  regResource.editAnswer(dataToPass).then(response => {
    if (response && response != 0) {
...
<template>
        ...
        <el-row>
            <el-col :span="12">
                <el-card v-if="formData && formData.answers">
                  ...
                  <!-- <el-form class="formDataClass" :model="formData" label-width="auto" label-position="left">
                    <el-form-item v-for="(answer, index) in filteredAnswers" :key="index" :label="answer.question" :class="{ 'light-gray-background': index % 2 === 0 }">
                      <el-select v-if="(answer.type_question === 'select')" v-model="answer.answer" :filterable="true" placeholder="Selecionar" @change="editTraje(answer)">
                        <el-option
                            v-for="answer in answer.possible_answers"
                            :key="answer.id"
                            :label="answer.description"
                            :value="answer.id"
                        />
                      </el-select>
                      <span v-else class="opensansName">{{ answer.answer != 'null' ? answer.answer : '--' }}</span><span :class="{'dangerSpan': !formData.isOlder, 'successSpan': formData.isOlder, 'opensansName' : true}" v-if="answer.cur_age">({{ answer.cur_age }} anos)</span>
                    </el-form-item>
                  </el-form> -->
                  <el-form ref="formRef" class="formContainer" :model="answers" label-width="auto" label-position="left">
                    <div v-for="(question, idx) in questions" :key="idx">
                      <el-form-item v-if="conditions[question.id] && question.type_question !== 'image'" :label="question.description" :prop="question.id.toString()" class="opensansName">
                          ...
                          />
                          <!-- Country -->
                          <el-select style="width: 100%" class="opensansName" v-if="(question.type_question === 'country' || question.type_question === 'district' || question.type_question === 'concelho' || question.type_question === 'freguesia' || question.type_question === 'select')  && conditions[question.id]" v-model="answers[question.id]" :filterable="checkFilterable(question)" clearable placeholder="Selecionar">
                            <el-option
    ...
    <el-dialog v-model="imageDialog" width="50%" :before-close="handleClose">
      <el-image
